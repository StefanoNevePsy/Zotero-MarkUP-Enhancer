...
zsem-prefs-apple-desc = macOS 27 ships /usr/bin/fm. On macOS 26 you need a third-party CLI (e.g. fmx): point this at it. In the command template {cli}, {prompt} and {out} are substituted; output is redirected to a file because Zotero cannot read a process's stdout.
